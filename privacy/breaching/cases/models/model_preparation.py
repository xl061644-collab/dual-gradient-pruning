...
def _construct_vision_model(cfg_model, cfg_data, pretrained=True, **kwargs):
    ...
    model_init_seed = 3618214207
    # np.random.randint(0, 2 ** 32 - 10)
    #
    set_random_seed(model_init_seed)

    print("initialize with seed ="+str(model_init_seed))
    ...
    if "ImageNet" in cfg_data.name:
        ...
        elif "lenet_zhu1" == cfg_model.lower():
            model = LeNetZhu1(num_channels=channels, num_classes=classes)
...
class LeNetZhu(torch.nn.Module):
    ...
    def forward(self, x):
        out = self.body(x)
        w = out.view(out.size(0), -1)

        # print("self.feature",self.feature)
        w= self.fc(w)
        return w

class LeNetZhu1(torch.nn.Module):
    """LeNet variant from https://github.com/mit-han-lab/dlg/blob/master/models/vision.py."""
    ...
    def __init__(self, num_classes=100, num_channels=3):
        ...
        self.fc = torch.nn.Sequential(torch.nn.Linear(768, num_classes))
        self.VB = VariationalBottleneck((768,))
        for module in self.modules():
            self.weights_init(module)
