...
# def cupy_topk(input_gradient,k,device='cpu'):
#     print("###############k#####################")
...
def cupy_topk(input_gradient,k,device):
    # print(type(input_gradient))
    # 先去除5%Top后保留的大参数
    input_gradient=list(input_gradient)
    for i in range(len(input_gradient)):
        grad_tensor = input_gradient[i].cpu().numpy()
        flattened_weights =np.abs(grad_tensor.flatten())
        thresh = np.percentile(flattened_weights, 90)
...
class UserSingleStep(torch.nn.Module):
    ...
    def plot1(self, user_data, scale=False, print_labels=False):
        """Plot user data to output. Probably best called from a jupyter notebook."""
        import matplotlib.pyplot as plt  # lazily import this here
        ...
        if data.shape[0] == 1:
            ...
            # if print_labels:
            #     print(label_classes)
    def plot2(self, user_data, scale=False, print_labels=False,name="a"):
        """Plot user data to output. Probably best called from a jupyter notebook."""
        import matplotlib.pyplot as plt  # lazily import this here
        import time
        ...
        if data.shape[0] == 1:
            plt.axis("off")
            plt.imshow(data[0].permute(1, 2, 0).cpu())
            plt.savefig('/home2/xl/breaching_ATS/results/' + name + ".jpg",bbox_inches='tight',dpi=fig.dpi,pad_inches=0.0)
            plt.show()
            if print_labels:
                plt.title(f"Data with label {classes[labels]}")
            ...
            fig.savefig('/home2/xl/breaching_ATS/results/'+name+".jpg")
            from PIL import Image
            fig = plt.figure()
            image = Image.open(
                '/home2/xl/breaching_ATS/results/'+name+".jpg")
            plt.imshow(image)
            plt.show()
            #
            #
    ...
    def plot4(self, user_data, scale=False, print_labels=False, name="a"):
        """Plot user data to output. Probably best called from a jupyter notebook."""
        import matplotlib.pyplot as plt  # lazily import this here
        ...
        if data.shape[0] == 1:
            plt.axis("off")
            plt.imshow(data[0].permute(1, 2, 0).cpu())
            plt.savefig(name, format='pdf', dpi=700, bbox_inches="tight")
            plt.show()
            if print_labels:
                plt.title(f"Data with label {classes[labels]}")
        else:
            ...
            fig.savefig(name, format='pdf', dpi=700, bbox_inches="tight")
            plt.show()

    def plot3(self, user_data, scale=False, print_labels=False, name="a"):
        """Plot user data to output. Probably best called from a jupyter notebook."""
        import matplotlib.pyplot as plt  # lazily import this here
        ...
        if data.shape[0] == 1:
            ...
            for axis in axes.flatten():
                axis.axis("off")
            fig.savefig(name, format='png', dpi=700, bbox_inches="tight")
            plt.show()
...
class UserMultiStep(UserSingleStep):
    ...
    def __init__(self, model, loss, dataloader, setup, idx, cfg_user):
        ...
        print("UserMultiStep(UserSingleStep)")
    ...
    def compute_local_updates1(self, server_payload):
        """Compute local updates to the given model based on server payload."""
        self.counted_queries += 1
        user_data = self._load_data()
...
class MultiUserAggregate(UserMultiStep):
    ...
    def __init__(self, model, loss, dataloaders, setup, cfg_user, user_indices):
        ...
        print("MultiUserAggregate(UserMultiStep)")
    ...
    def compute_local_updates(self, server_payload, return_true_user_data=True):
        """Compute local updates to the given model based on server payload.
        ...
        for user in self.users:
            ...
            # user_data['gradients'] = cupy_topk(user_data['gradients'], 50,'cpu')
            user.to(**self.user_setup)
            user.setup = self.user_setup

            if return_true_user_data:
                aggregate_true_user_data += [true_user_data["data"]]
